...
test('health and readiness expose only safe pilot state while every Live lock stays active', () => {
  const env = safeEnv();
...
test('the committed pilot remains disarmed and blocks confirmed scheduled or order work', () => {
...
test('Durable Object audit summarizes scanner, expiration, duplicate, broker failure, and protected submission events', async () => {
  const env = safeEnv();
  ...
  await recordSandboxPilotEvent(storage, {
    type: 'SANDBOX_ORDER_REJECTED',
    status: 'REJECTED',
    brokerHost: 'https://api.sandbox.webull.com',
    brokerStatus: 503,
    protectedOrder: true,
    executionAttempted: true,
    reason: 'sandbox broker unavailable',
  }, { now: NOW - 1_000 });
  await recordSandboxPilotEvent(storage, {
  ...
  assert.equal(audit.liveFundsUsed, false);
});

test('order status reflects reservations and enforces the one-submission pilot ceiling', async () => {
  const env = safeEnv();
  const storage = memoryStorage();
...
test('Sandbox pilot configuration, entrypoint, endpoints, observability, and secret hygiene stay locked', () => {
  ...
  for (const path of [SANDBOX_HEALTH_PATH, SANDBOX_READINESS_PATH, SANDBOX_AUDIT_PATH, SANDBOX_ORDERS_STATUS_PATH]) {
    assert.ok(dashboardEntry.includes(path.replace('/api/', 'SANDBOX_')) || dashboardEntry.includes(path));
  }
