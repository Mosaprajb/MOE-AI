...
(function(){
  ...
  function selectedStrategies(){return [...document.querySelectorAll('input[name="sim-strategy"]:checked')].map(node=>node.value);}
  ...
  function render(){
    ensureBanner();const panel=ensurePanel();const active=latest.active===true;document.body.classList.toggle('moe-simulation-active',active);
    const selected=(latest.selectedStrategies||[]).map(badge).join(' ');
    ...
    panel.innerHTML='<div class="sim-head"><div><div class="sim-kicker">HISTORICAL REPLAY · LOCAL BROKER</div><h3 class="sim-title">Simulation Mode</h3></div><span class="sim-safe">LIVE LOCKED · ZERO WEBULL REQUESTS</span></div>'+
      '<div class="sim-grid"><div class="sim-box"><span class="sim-label">STRATEGIES FOR THIS RUN</span><div class="sim-options"><label class="sim-option"><input type="checkbox" name="sim-strategy" value="FUSION_V2" checked '+(active?'disabled':'')+'> FUSION_V2</label><label class="sim-option"><input type="checkbox" name="sim-strategy" value="MOERAND_SIMPLE_INTERNAL" checked '+(active?'disabled':'')+'> MOERAND_SIMPLE_INTERNAL</label></div></div>'+
      '<div class="sim-box"><label class="sim-label" for="sim-range">HISTORICAL RANGE</label><select class="sim-select" id="sim-range" '+(active?'disabled':'')+'><option value="LAST_SESSION">Last completed session</option><option value="LAST_3_DAYS">Last 3 completed sessions</option></select></div>'+
      '<div class="sim-box"><label class="sim-label" for="sim-speed">REPLAY SPEED</label><select class="sim-select" id="sim-speed" '+(active?'disabled':'')+'><option value="60">60x — one 5m candle every 5s</option><option value="300">300x — one 5m candle every 1s</option></select></div>'+
      ...
      '<div class="sim-status"><div class="sim-chip">STATUS<b class="'+(active?'sim-running':'')+'">'+esc(latest.status||'IDLE')+'</b></div><div class="sim-chip">SIMULATING<b>'+(selected||'—')+'</b></div><div class="sim-chip">SIMULATED TIME<b>'+esc(latest.simulatedAt?new Date(latest.simulatedAt).toLocaleString():'—')+'</b></div><div class="sim-chip">SPEED<b>'+esc(latest.speedMultiplier||60)+'x</b></div><div class="sim-chip">PROGRESS<b>'+esc(latest.progressPercent||0)+'%</b></div><div class="sim-chip">BROKER<b>LOCAL ONLY</b></div></div>'+
  ...
  async function refresh(){try{const payload=await jsonRequest(endpoints.status);latest=payload.simulation||payload;errorText='';render();}catch(error){errorText=error.message||String(error);render();}}
  async function start(){try{const pin=document.getElementById('sim-pin')?.value||'';const strategies=selectedStrategies();if(!strategies.length)throw new Error('Select at least one strategy.');await jsonRequest(endpoints.session,{method:'POST',body:JSON.stringify({pin})});const pinNode=document.getElementById('sim-pin');if(pinNode)pinNode.value='';const range=document.getElementById('sim-range')?.value||'LAST_SESSION';const speedMultiplier=Number(document.getElementById('sim-speed')?.value||60);const payload=await jsonRequest(endpoints.start,{method:'POST',body:JSON.stringify({strategies,range,speedMultiplier})});latest=payload.simulation;lastTickAt=0;errorText='';render();}catch(error){errorText=error.message||String(error);render();}}
