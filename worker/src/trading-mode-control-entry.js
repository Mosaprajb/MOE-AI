...
const BUILD_ID = 'trading-mode-control-v1-20260727';
...
function json(payload, status = 200) {
  return Response.json(payload, {
    ...
    headers: {
      ...
      'x-moe-trading-control': BUILD_ID,
    },
  });
}
...
async function createLiveSession(env) {
  ...
  return { token: `${body}.${signature}`, expiresAt: new Date(payload.expiresAt).toISOString(), ttlMinutes };
}
...
async function verifyLiveSession(request, env) {
  ...
  if (!constantTimeEqual(expected, supplied)) return { ok: false, code: 'LIVE_SESSION_INVALID' };
  try {
...
function liveEnvironment(env) {
  return {
    ...
    MOE_LIVE_MAX_ORDER_NOTIONAL: env.WEBULL_LIVE_MAX_NOTIONAL || env.MOE_LIVE_MAX_ORDER_NOTIONAL,
  };
}

function assertLiveFlags(env) {
  const blockers = [];
...
async function unlock(request, env) {
  ...
  if (!enabled(env.MOE_LIVE_PIN_CONTROL_ENABLED)) return json({ ok: false, code: 'LIVE_PIN_DISABLED' }, 403);
  ...
  if (!suppliedPin || !constantTimeEqual(expectedDigest, suppliedDigest)) {
    console.warn(JSON.stringify({ event: 'LIVE_MODE_UNLOCK_REJECTED', createdAt: new Date().toISOString() }));
    return json({ ok: false, code: 'LIVE_PIN_INVALID', error: 'Invalid live trading PIN' }, 403);
  }
  const blockers = assertLiveFlags(env);
...
async function execute(request, env) {
  ...
  if (mode === 'sandbox') {
    return json({
      ok: true,
      mode: 'SANDBOX',
      execution: 'ROUTED_TO_EXISTING_SANDBOX_PIPELINE',
  ...
  if (body.confirm !== true) {
    return json({ ok: true, mode: 'LIVE', executionAttempted: false, confirmationRequired: true, preview });
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === LOCK_PATH && request.method === 'POST') {
        console.log(JSON.stringify({ event: 'LIVE_MODE_LOCKED', createdAt: new Date().toISOString() }));
        return json({ ok: true, mode: 'SANDBOX', liveSessionActive: false, clientMustDiscardSessionToken: true });
      }
