...
export function createEngineSignal(input = {}) {
  ...
  const confidenceIsObject =
    input.confidence != null &&
    typeof input.confidence === 'object' &&
    !Array.isArray(input.confidence);

  const confidenceValue = confidenceIsObject
    ? input.confidence.value
    : input.confidence ?? input.score ?? 0;

  const confidenceSource = confidenceIsObject
    ? (input.confidence.source ?? input.confidenceSource ?? input.engine ?? 'engine')
    : (input.confidenceSource ?? input.engine ?? 'engine');
  ...
  return Object.freeze({
    ...
    confidence: createConfidence(confidenceValue, confidenceSource),
    reasons,
    diagnostics,
