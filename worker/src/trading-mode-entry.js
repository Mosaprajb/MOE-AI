...
async function enforceMode(request, env) {
  ...
  if (mode.effectiveMode === TRADING_MODES.LIVE) {
    if (!live.control.effectiveLiveUnlocked) return secureJson({ ok: false, blocked: true, submitted: false, error: 'Live controls are locked or the kill switch is active.' }, 423);
