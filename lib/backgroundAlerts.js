...
export async function subscribeBackgroundAlerts({ serviceWorkerPath, symbols, timeframe }) {
  const registration = await readyRegistration(serviceWorkerPath);
  ...
  await apiRequest('/api/subscribe', {
    method: 'POST',
    body: JSON.stringify({ subscription: subscription.toJSON(), symbols, timeframe })
  });
  return subscription;
}

export async function syncBackgroundAlerts({ serviceWorkerPath, symbols, timeframe }) {
