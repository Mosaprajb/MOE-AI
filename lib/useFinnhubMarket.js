...
async function fetchAlpacaHistory(stocks, key, secret, from, to) {
...
export function useFinnhubMarket(seedStocks) {
  ...
  const loadCandleHistory = useCallback(async (token) => {
    ...
    if (!failedCount) {
      ...
      setEngineMessage(`MOE v${MOE_VERSION} active · ${provider} 1m history · 15m context`);
  ...
  const saveAlpacaCredentials = useCallback(async (key, secret) => {
    ...
    setMarketStocks((current) => current.map((stock) => waitingStock(stock, 'LOADING ALPACA CANDLE HISTORY')));
