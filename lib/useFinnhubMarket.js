...
export function useFinnhubMarket(seedStocks) {
  ...
  const loadCandleHistory = useCallback(async (token) => {
    ...
    const results = await Promise.allSettled(seedStocks.map(async (stock) => {
      const response = await fetch(
    ...
    const readySymbols = results
      .filter((result) => result.status === 'fulfilled')
      .map((result) => result.value);
    const failedCount = results.length - readySymbols.length;
  ...
  const connect = useCallback(async (token, persist = true) => {
    ...
    setMarketStocks((current) => current.map((stock) => waitingStock(stock)));
    setStatus('connecting');
    setStatusMessage('Loading the latest market quotes…');

    connectSocket(cleanToken);
    await Promise.allSettled([
      loadInitialQuotes(cleanToken),
      loadCandleHistory(cleanToken)
    ]);
    return true;
  }, [connectSocket, loadCandleHistory, loadInitialQuotes]);
